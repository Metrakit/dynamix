...
<div class="comment-user-footer">
	...
	{{ Form::open(array('url' => 'comment/' . $comment->id . '/vote/1', 'class' => 'comment-vote comment-vote-up display-inline-block')) }}
        <button type="submit" class="button-transparent" title="{{{ Lang::get('comment.vote_up') }}}"><span class="comment-vote-up glyphicon glyphicon-chevron-up {{(Auth::check()?($comment->userHasVotePositive(Auth::user()->id)?'color-blue':''):'')}}"></span></button>
    {{ Form::close() }}
     | 
