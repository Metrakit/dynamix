<div class="menu-object menu-object-{{$role}}">
	<div class="buttons">
		...
		{{ Form::close() }}
		<div class="pull-right"><a href="{{URL::to('admin/navigation/' . $nav->id . '/edit')}}" class="btn btn-primary btn-xs pencil"><span class="glyphicon glyphicon-pencil"></span></a></div>
	...
	<div class="menu-object-{{$role}}-content">
		...
		<div class="content {{(!$nav->hasResource() ?'text-danger':'')}}">
			{{$nav->title()}} <small>({{$nav->navigable_type}})</small>
		</div>
