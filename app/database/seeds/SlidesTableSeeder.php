...
class SlidesTableSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        $title1->save();
        $title1->translate('fr','Chatton mignon 1');
        $title1->translate('en','Cute cat 1');

        $description1 = new I18N;
        $description1->i18n_type_id = I18nType::where('name','=','title')->first()->id;
        $description1->save();
        $description1->translate('fr','Un petit chatton tout mignon');
        $description1->translate('en','A very cutest kitten');


        $title2 = new I18N;
        $title2->i18n_type_id = I18nType::where('name','=','title')->first()->id;
        $title2->save();
        $title2->translate('fr','Chatton mignon 2');
        $title2->translate('en','Cute cat 2');

        $description2 = new I18N;
        $description2->i18n_type_id = I18nType::where('name','=','title')->first()->id;
        $description2->save();
        $description2->translate('fr','Un petit chatton tout mignon');
        $description2->translate('en','A very cutest kitten');
